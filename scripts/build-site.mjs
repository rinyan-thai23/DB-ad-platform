...
function categoryPage(c) {
  ...
  const body = `<div class="wrap breadcrumb"><a href="${href('/')}">トップ</a> / <a href="${href('/categories/')}">カテゴリ</a> / ${esc(c.title)}</div><section class="page-intro"><div class="wrap"><p class="eyebrow">${esc(c.group)} · ${list.length} services</p><h1>${esc(c.title)}</h1><p class="lead">${esc(c.description)}</p></div></section><section class="section category-results" data-category-context><div class="wrap"><div class="category-context"><div><p class="eyebrow">Selection rule</p><h2>このジャンルの判定</h2><p>${esc(c.description)}</p><p class="category-count"><strong>${list.length}</strong>件が現在のDB条件に一致しています。</p></div><aside><p class="eyebrow">Related genres</p><div class="related-links">${siblings.length?siblings.map(category=>`<a href="${href(`/categories/${category.slug}/`)}">${esc(category.title)} <span>${rows.filter(category.filter).length}</span></a>`).join(''):`<a href="${href('/categories/')}">全ジャンルを見る</a>`}</div></aside></div><div class="section-head results-heading"><div><p class="eyebrow">Matched services</p><h2>${esc(c.title)}の比較一覧</h2></div><a class="text-link" href="${href('/categories/')}">全ジャンルを見る</a></div><div class="cards">${list.map(card).join('')}</div></div></section>`;
