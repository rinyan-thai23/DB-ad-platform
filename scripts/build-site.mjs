...
const categories = [
  ...
  { group:'サイト・ホスティング', slug:'custom-domain-required', title:'独自ドメイン必須', short:'独自ドメインでの運営が前提', description:'無料URLの公式・条件付き・技術推定候補が確認できず、独自ドメイン必須として保守的に分類したサービス。', filter:r=>r.custom_domain_category==='独自ドメイン必須' },
  { group:'サイト・ホスティング', slug:'no-custom-domain', title:'独自ドメインなしでも候補', short:'無料URLから検討できる', description:'5種類の無料URLのうち、1つ以上が公式OK・条件付き・技術推定OKとなるサービス。媒体審査の通過を保証する分類ではありません。', filter:r=>r.custom_domain_category==='独自ドメインなしでも候補' },
...
function tags(r) {
  ...
  const domainLabel = r.custom_domain_category === '独自ドメイン必須' ? '独自ドメイン: 必須' : '独自ドメイン: なしでも候補';
...
function card(r) {
  ...
  const positiveHost = value => ['公式OK','条件付き','技術推定OK'].includes(value);
  const hosting = [['blogspot',r.blogspot_free_url],['wordpress',r.wordpress_com_free_url],['github',r.github_pages_free_url],['vercel',r.vercel_free_subdomain],['cloudflare',r.cloudflare_pages_free_subdomain]].filter(([,value])=>positiveHost(value)).map(([key])=>key).join(' ');
  const payments = [
...
function homePage() {
  ...
  const body = `<section class="hero"><div class="wrap hero-grid"><div><p class="eyebrow">日本語サイト向け広告データベース・2026年版</p><h1>広告を、単価ではなく<br><em>始めやすさ</em>で選ぶ。</h1><p class="lead">日本語サイト、無料URL、日本の銀行口座。あなたの今の条件から、使える広告プラットフォームを絞り込みます。</p></div><aside class="hero-panel"><div class="big-number">30</div><div class="metric-label">広告サービスを同一基準で比較</div><p class="metric-sub">60以上の調査項目／公式情報と技術推定を分離／毎月更新を想定</p></aside></div></section>
...
function servicePage(r) {
  ...
  <section class="section"><div class="wrap detail-layout"><div><div class="panel genre-panel" data-related-genres><div class="panel-heading"><div><p class="eyebrow">該当ジャンル・${relatedCategories.length}件</p><h2>このサービスが属するジャンル</h2></div><a class="text-link" href="${href('/categories/')}">全ジャンルを見る</a></div><div class="genre-list">${relatedCategories.map(category=>`<a class="genre-chip" href="${href(`/categories/${category.slug}/`)}"><small>${esc(category.group)}</small><strong>${esc(category.title)}</strong></a>`).join('')}</div></div><div class="panel"><h2>独自ドメイン・無料URL</h2><div class="domain-verdict"><span>サイト種類の大分類</span><strong>${esc(r.custom_domain_category)}</strong><p>${r.custom_domain_category==='独自ドメインなしでも候補'?'無料URLのいずれかに候補があります。媒体審査の通過を保証するものではありません。':'無料URLの候補を確認できないため、独自ドメイン必須として保守的に分類しています。'}</p></div><h3 class="subheading">無料URLごとの判定</h3><dl class="facts">${Object.entries(hostLabels).map(([k,v])=>`<dt>${v}</dt><dd>${esc(r[k])}</dd>`).join('')}</dl><p class="notice">「技術推定OK」はタグを置ける可能性を示すもので、広告会社による媒体承認の保証ではありません。</p></div>${sections.map(([name,fields])=>`<div class="panel"><h2>${name}</h2>${factList(r,fields)}</div>`).join('')}<div class="panel"><h2>一次情報・参照先</h2>${sources.length?`<ul class="sources">${sources.map(u=>`<li><a href="${esc(u)}" rel="nofollow noopener" target="_blank">${esc(u)}</a></li>`).join('')}</ul>`:'<p>公開一次情報のURLは確認できていません。</p>'}${r.notes?`<p class="notice">${esc(r.notes)}</p>`:''}</div></div><aside class="sticky"><div class="panel"><p class="eyebrow">公式サイト</p><h2>${esc(r.service_name)}を確認</h2><p>申込前に最新条件を公式サイトまたは管理画面で再確認してください。</p><a class="cta" href="${esc(outbound)}" rel="sponsored nofollow noopener" target="_blank">公式サイトへ進む</a><p class="disclosure">affiliate_urlが設定されている場合は、成果報酬リンクへ自動的に切り替わります。</p></div></aside></div></section>`;
...
function categoriesPage() {
  ...
  const body = `<section class="page-intro"><div class="wrap"><p class="eyebrow">条件から探す</p><h1>条件別カテゴリ</h1><p class="lead">サイト規模、受取方法、無料URL、広告形式から${categories.length}ジャンルに絞り込めます。</p></div></section><section class="section"><div class="wrap">${groupedCategories}</div></section>`;
...
function infoPage(kind) {
  ...
  const body = guide ? `<section class="page-intro"><div class="wrap prose"><p class="eyebrow">初心者向けガイド</p><h1>${title}</h1><p class="lead">単価より先に、審査・設置・出金の3つを確認すると失敗を減らせます。</p><h2>1. まず「承認されるか」を確認</h2><p>広告タグを設置できても、無料サブドメインが媒体として承認されるとは限りません。本DBは公式確認と技術推定を分けています。</p><h2>2. 出金経路を確認</h2><p>最低支払額だけでなく、日本の銀行までPayPalやPayoneerなしで到達できるかを確認してください。国際銀行送金は中継銀行手数料が発生する場合があります。</p><h2>3. 読みやすさに合う広告形式を選ぶ</h2><p>Pop・Pushは収益化しやすい一方で、読者の操作を妨げやすい形式です。長期運営する日本語サイトではバナー・ネイティブ広告から試すのが無難です。</p></div></section>` : `<section class="page-intro"><div class="wrap prose"><p class="eyebrow">調査方法</p><h1>${title}</h1><p class="lead">日本語サイト初心者の「0円から初収益」を支えるための比較DBです。</p><h2>評価の優先順位</h2><p>広告単価ランキングではありません。初心者適性、日本語サイトへの適合、日本の銀行口座までの出金、無料サブドメイン・静的サイト適合を重視します。</p><h2>情報の確度</h2><p>公式ページで確認できた事実と、HTML・JavaScriptの設置可否から判断した技術推定を分離します。不明な項目は推測で埋めません。</p><h2>更新</h2><p>原則として月1回、変更リスクHighのサービスから公式ページを確認し、TSV更新後に全HTMLを再生成します。最終確認日は各サービスページに表示します。</p><h2>免責</h2><p>審査・支払方法・最低額はアカウント、国、時期により変わる場合があります。申込前に必ず公式情報を確認してください。</p></div></section>`;
