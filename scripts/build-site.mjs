...
function servicePage(r) {
  ...
  const editorial = r.pros && r.cons && r.recommended_for ? `<div class="editorial-grid" data-editorial-review><section class="editorial-card editorial-pros"><p class="eyebrow">PROS</p><h2>メリット</h2><p>${esc(r.pros)}</p></section><section class="editorial-card editorial-cons"><p class="eyebrow">CONS</p><h2>注意点</h2><p>${esc(r.cons)}</p></section><section class="editorial-card editorial-fit"><p class="eyebrow">BEST FOR</p><h2>こんな人におすすめ</h2><p>${esc(r.recommended_for)}</p></section></div>` : '';
